...
function HomePage() {
    ...
    return <div className="w-screen h-screen flex flex-col gap-20 justify-center items-center text-green-700">
      ...
      <div className='bg-neutral-900 rounded-2xl p-5 flex'>
        
        <div className={`flex flex-col gap-2 transition-all duration-1000 ease-in-out ${!showPlayMenu ? '' : '-translate-x-[125%] opacity-0'}`}>
          <button onClick={() => setPlayMenu(true)} className='grid grid-cols-3 place-items-center hover:bg-neutral-950 rounded-lg p-2'>
            <img src={gameIcon} className="w-12 aspect-square" />
            <h2 className='font-bold text-2xl col-span-2'>Play a Game</h2>
          </button>
          <Link to="/" className={'grid grid-cols-3 place-items-center hover:bg-neutral-950 rounded-lg p-2' + disabledStyle}>
            <img src={puzzleIcon} className="w-12 aspect-square" />
            <h2 className='font-bold text-2xl col-span-2'>Puzzles</h2>
          </Link>
          <Link to="/documentation" className='grid grid-cols-3 place-items-center hover:bg-neutral-950 rounded-lg p-2'>
            <img src={documentationIcon} className="w-12 aspect-square" />
            <h2 className='font-bold text-2xl col-span-2'>Documentation</h2>
          </Link>
          <Link to="/" className={'grid grid-cols-3 place-items-center hover:bg-neutral-950 rounded-lg p-2' + disabledStyle}>
            <img src={accountIcon} className="w-12 aspect-square" />
            <h2 className='font-bold text-2xl col-span-2'>Account</h2>
          </Link>
        </div>

        <div className={`flex flex-col gap-2 z-0 absolute transition-all duration-1000 ease-in-out ${!showPlayMenu ? 'translate-x-[125%] opacity-0' : ''}`}>
          <button onClick={() => setPlayMenu(false)} className='grid grid-cols-3 place-items-center hover:bg-neutral-950 rounded-lg p-2'>
            <img src={backIcon} className="w-12 aspect-square" />
            <h2 className='font-bold text-2xl col-span-2'>Back</h2>
          </button>
          <Link to="/play" className='grid grid-cols-3 place-items-center hover:bg-neutral-950 rounded-lg p-2'>
            <img src={accountIcon} className="w-12 aspect-square" />
